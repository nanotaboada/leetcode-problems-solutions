...
/*
    ...
    https://leetcode.com/problems/product-of-array-except-self/solutions/4508578/java-time-and-space-complexity-0-n-runtime-1ms-beats-100-step-by-step-explanation/

    - Runtime 1 ms (Beats 100.00% of users with Java)
    - Memory 54.81 MB (Beats 5.82% of users with Java)
*/
...
class Solution {
    public int[] productExceptSelf(int[] nums) {
        ...
        // Cycle the collection left-to-right (from the second element to the last),
        for (int leftToRight = 1; leftToRight < nums.length; leftToRight++) {
            // calculating the poduct of the elements to its left.
            left[leftToRight] = left[leftToRight - 1] * nums[leftToRight - 1];
        }
        // Cycle the collection right-to-left (from the second to last to the first),
        for (int rightToLeft = nums.length - 2; rightToLeft >= 0; rightToLeft--) {
            // calculating the poduct of the elements to its right.
